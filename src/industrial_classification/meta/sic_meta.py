"""Module that defines the SIC metadata structure aligned with 2007.

This module contains the SIC metadata structure for the 2007 version of the
classification. The structure is a dictionary with the SIC code as the key and
a dictionary with the metadata as the value.
"""
...
class SicMeta:
    ...
    def __init__(self, retrofit_keys=False):
        """Initialize the SicMeta class.

        Args:
            self (dict): Dictionary containing SIC metadata.
            retrofit_keys (bool): Flag to retrofit keys by removing the first character.
        """
        if retrofit_keys:
            self.sic_meta = {key[1:]: value for key, value in SICmeta.items()}
        else:
            self.sic_meta = sic_meta

    def get_meta_by_code(self, code: str) -> dict:
        """Retrieve title and detail for a given SIC code.

        Args:
            code (str): A 2 or 5 digit SIC code to lookup.
        ...
        # Look for the code in the metadata, when 5 digits are not found
        # look for 4 and so on
        for _ in range(len(code)):
            # Check for an exact match
            if code in self.sic_meta:
                entry = self.sic_meta[code]
                return {
                    "code": code,
            ...
            # Replace the last non-X character with 'x'
            code_list = list(code)
